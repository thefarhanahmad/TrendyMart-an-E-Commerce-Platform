  ...
  return (
    <div>
      <div className='py-3 flex flex-col items-center justify-between w-full gap-3 p-4 transition  ease-in rounded-xl border-2 border-[#00095] shadow-lg hover:shadow-2xl hover:scale-[1.03] md:hover:scale-[1.05] h-[375px] bg-[#faf5ff]'>
        <div>
          <p className='text-[#9333ea] font-semibold text-lg '>{items.title.split(" ").slice(0, 3).join(" ") + "..."}</p>
        </div>
        ...
        <div className='flex items-center justify-between w-full mt-5'>
          <div>
            <p className='font-semibold text-[#9333ea]'>${items.price}</p>
          </div>
          {
            cart.some((p) => p.id == items.id) ?
              (
                <button className='font-semibold text-[#9333ea] border-2 border-[#9333ea] rounded-full text-[12px] p-1 px-3 uppercase
                 hover:bg-[#9333ea] hover:text-white duration-300' onClick={removeFromCart}>
                  Remove Item
                </button>
              ) :
              (
                <button className='font-semibold text-[#9333ea] border-2 border-[#9333ea] rounded-full text-[12px] p-1 px-3 uppercase
                 hover:bg-[#9333ea] hover:text-white duration-300' onClick={addToCart}>
                  Add To Cart
                </button>
              )
          }
